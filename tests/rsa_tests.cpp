...
TEST(ModArith, ModExp) {
  ...
  rsa::uint2048_t result = rsa::mod_exp(base, exp, mod);
...
TEST(ModArith, ExtendedGCD) {
  ...
  rsa::uint2048_t b = 46;
  rsa::gcd_combo result = rsa::extended_gcd(a, b);
...
TEST(ModArith, ModInverse) {
  ...
  rsa::uint2048_t m = 37;
  rsa::uint2048_t result = rsa::mod_inverse(a, m);
...
TEST(ModArith, FermatLittleTheorem) {
  ...
  rsa::uint2048_t a(2);
  rsa::uint2048_t result = rsa::mod_exp(a, p - rsa::uint2048_t(1), p);
  EXPECT_EQ(result, rsa::uint2048_t(1));
}

TEST(PrimeGen, LowLevelPrime) {
  rsa::uint2048_t prime = rsa::generate_low_level_prime();
...
TEST(PrimeGen, KnownSmallPrimes) {
  ...
  EXPECT_FALSE(rsa::is_composite(n, rsa::uint2048_t(2), d, r));
  EXPECT_FALSE(rsa::is_composite(n, rsa::uint2048_t(3), d, r));
  EXPECT_FALSE(rsa::is_composite(n, rsa::uint2048_t(5), d, r));
}
...
TEST(PrimeGen, KnownComposite) {
  ...
  EXPECT_TRUE(rsa::is_composite(n, rsa::uint2048_t(2), d, r));
  EXPECT_TRUE(rsa::is_composite(n, rsa::uint2048_t(3), d, r));
  EXPECT_TRUE(rsa::is_composite(n, rsa::uint2048_t(5), d, r));
}

TEST(PrimeGen, RabinMillerKnownPrimes) {
  rsa::uint2048_t prime_a(15487469);
  rsa::uint2048_t prime_b(7867);
  EXPECT_TRUE(rsa::rabin_miller_test(prime_a));
  EXPECT_TRUE(rsa::rabin_miller_test(prime_b));
}
...
TEST(PrimeGen, RabinMillerBigPrime) {
  ...
  EXPECT_TRUE(rsa::rabin_miller_test(prime));
}

TEST(PrimeGen, LargePrime) {
  rsa::uint2048_t prime = rsa::generate_large_prime();
  EXPECT_TRUE(prime.get_bit(1023)); // Ensure it's 1024 bits
  EXPECT_TRUE(prime.get_bit(0));    // Ensure it's odd
}
...
TEST(RSA, EncryptDecrypt) {
  ...
  rsa::uint2048_t ciphertext = rsa::encrypt(message, pub_key);
  rsa::uint2048_t decrypted_message = rsa::decrypt(ciphertext, priv_key);
